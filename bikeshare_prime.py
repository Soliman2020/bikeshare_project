...
def time_stats(df):
    ...
    print("\nProcess took {0:.2f} seconds.".format(time.time() - start_time))
    print('-'*40)
...
def user_stats(df,city):
    ...
    if city!='Washington':
        ...
        print('\n_Earliest, most recent, and most common year of birth:_\n')
...
def top_df(df):
    ...
    ''' Print asked top rows of the DataFrame. '''

    while True:
        desire = input('Do you want to see top lines of raw data? Enter yes or no. >>  ')
        if desire.lower() != 'yes':
                break
        else:
            try:
                rows=int(input('\nEnter the number of top rows you wanna give a look:  '))
                assert rows>0
                start_time = time.time()
                x = df.head(rows)
                ...
                print('Attention! You should enter a number. Let\'s say it\'s 10 :D \n')
            except AssertionError:
                print('You wanna see an empty DataFrame!!')
...
def main():
    while True:
        ...
        if restart.lower() != 'yes':
            print('Thanks.. Mission Accomplished :D')
            break
